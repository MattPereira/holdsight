...
export const PLAID_BROKERAGE_PROVIDER = "plaid";
 ...
 */
export async function getPlaidHoldings(
  accessToken: string,
  ...
  try {
    ...
    // The user's login expired/changed, so they must re-link via Plaid Link.
    if (code === "ITEM_LOGIN_REQUIRED") return { status: "login_required" };
    return { status: "error", message, httpStatus };
  }
}

export const plaidBrokerageProvider: BrokerageHoldingsProvider = {
  id: PLAID_BROKERAGE_PROVIDER,
  getHoldings(connection: BrokerageProviderConnection) {
    return getPlaidHoldings(connection.accessToken);
  },
};

const INVESTMENT_TRANSACTIONS_PAGE_SIZE = 500;

export type PlaidBrokerageTransactionsPageResult =
  | {
      status: "ready";
...
export async function getPlaidInvestmentTransactionsPage(
  accessToken: string,
  input: {
    startDate: string;
    endDate: string;
...
): Promise<PlaidBrokerageTransactionsPageResult> {
  try {
